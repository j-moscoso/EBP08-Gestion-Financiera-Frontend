...
export function IncomesPage() {
  ...
  const [editingIncome, setEditingIncome] = useState<ScheduledTransaction | null>(null);
  const [deletingIncome, setDeletingIncome] = useState<ScheduledTransaction | null>(null);
  ...
  const handleEdit = (income: ScheduledTransaction) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Gestiona tus ingresos programados y revisa el historial
          </p>
        </div>
        ...
        >
          ...
          <span>Nuevo Ingreso Programado</span>
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-card p-6 rounded-xl shadow-md border border-border">
          <h2 className="text-foreground mb-4">
            {editingIncome ? 'Editar Ingreso Programado' : 'Nuevo Ingreso Programado'}
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 bg-primary/10 border border-primary/20 rounded-lg">
              <p className="text-foreground text-sm">
                El sistema aplicará automáticamente este ingreso en la fecha programada y lo eliminará al llegar a la fecha final.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              >
                {editingIncome ? 'Actualizar' : 'Crear Ingreso Programado'}
              </button>
              <button
                type="button"
                onClick={() => {
      ...
      {activeTab === 'scheduled' ? (
        <div>
          <h2 className="text-foreground mb-4">Ingresos Programados</h2>
          {scheduledIncomes.length === 0 ? (
            <div className="bg-card p-8 rounded-xl border border-border text-center">
              <Calendar className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-foreground mb-2">No hay ingresos programados</h3>
              <p className="text-muted-foreground">
                Crea tu primer ingreso programado para automatizar tus registros
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          <h3 className="text-foreground">{income.description}</h3>
                          <p className="text-sm text-muted-foreground">
                            {category?.name || 'Sin categoría'} • {getFrequencyLabel(income.frequency)}
                          </p>
                          <p className="text-sm text-muted-foreground mt-1">
                            Del {formatDate(income.startDate)} al {formatDate(income.endDate)}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-primary">{formatCurrency(income.amount)}</p>
                        </div>
                      </div>
        ...
        <div>
          ...
          <h2 className="text-foreground mb-4">Ingresos Recientes</h2>
          {filteredIncomes.length === 0 ? (
            <div className="bg-card p-8 rounded-xl border border-border text-center">
              <TrendingUp className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-foreground mb-2">No hay ingresos registrados</h3>
              <p className="text-muted-foreground">
                Los ingresos que registres aparecerán aquí
              </p>
            </div>
          ) : (
            <>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h3 className="text-foreground">{income.description}</h3>
                          <p className="text-sm text-muted-foreground">
                            {category?.name || 'Sin categoría'} • {formatDate(income.date)}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-primary">{formatCurrency(income.amount)}</p>
      ...
      {deletingIncome && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-6 z-50">
          <div className="bg-card p-6 rounded-xl shadow-xl border border-border max-w-md w-full">
            <h2 className="text-foreground mb-4">¿Eliminar ingreso programado?</h2>
            <p className="text-muted-foreground mb-4">
              ¿Estás seguro de que deseas eliminar "{deletingIncome.description}"? Esta acción no generará futuros ingresos asociados.
            </p>
